...
impl Config {
    pub fn from_args() -> Self {
        ...
        let binary = env::args().next().unwrap_or("dolphinx".to_string());

        println!("Usage:");
        println!("{} <target> <connections> <concurrency> [hold] [infinite] [http] [rate N]", binary);

        if args.len() < 4 {
            println!("Usage:");
            println!(
                "multi-stresser <target> <connections> <concurrency> [hold] [infinite] [http] [rate N]"
            );
            std::process::exit(1);
        }
